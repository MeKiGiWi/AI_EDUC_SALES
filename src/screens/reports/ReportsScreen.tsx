...
const roleContent = {
  student: {
    ...
    emptyDescription:
      "Завершите диалог в тренажере — здесь появятся сохраненные отчеты с предпросмотром и выгрузкой.",
    infoLines: [
      "После каждого завершенного диалога сохраняется новый отчет.",
      "Экран загружает общий список отчетов из базы данных.",
  ...
  manager: {
    ...
    emptyDescription:
      "После завершения диалогов здесь появится общий список сохраненных отчетов.",
    infoLines: [
      "После сохранения отчет попадает в общую базу данных.",
      "Из этой вкладки доступны предпросмотр, PDF и CSV."
    ]
  },
  hr: {
    ...
    infoLines: [
      "Все сохраненные отчеты загружаются из базы данных.",
  ...
  admin: {
    ...
    infoLines: [
      "Экран показывает общий пул отчетов из базы данных.",
...
export function ReportsScreen({ activeRole, reports, highlightReportId }: ReportsScreenProps) {
  ...
  return (
    <>
      ...
      <AppCard tone="mint">
        <Text style={[styles.title, { color: theme.semantic.textPrimary }]}>База отчетов</Text>
        <Text style={[styles.body, { color: theme.semantic.textSecondary }]}>
          Здесь отображаются все отчеты, которые были успешно сохранены после завершения сценариев.
        </Text>
        <Text style={[styles.meta, { color: theme.semantic.textMuted }]}>
