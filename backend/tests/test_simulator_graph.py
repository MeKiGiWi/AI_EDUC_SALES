...
from app.prompts import BUYER_SYSTEM_PROMPT
...
async def test_graph_starts_with_scenario_context_and_opening_message() -> None:
    graph = create_graph(
        GraphDependencies(
            ...
            buyer_agent=BuyerAgent(RunnableLambda(lambda _: AIMessage(content="Ответ"))),
        )
    )
    ...
    assert "Среднее производственное предприятие" in started["messages"][1].content
    assert isinstance(started["messages"][2], AIMessage)
    assert (
        started["messages"][2].content
        == "Добрый день. Мы уже обсуждали проект, но я пока не готов двигаться дальше. Нужно еще раз все взвесить."
    )
