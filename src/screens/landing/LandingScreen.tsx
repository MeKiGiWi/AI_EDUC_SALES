...
const PLANS: PlanCard[] = [
  {
    ...
    price: "550 000 ₽",
    desc: "Полный контур развития навыков администраторов и контроля качества коммуникаций с кастомной настройкой под процессы клиники",
    includes: [
      ...
      "Кастомная настройка функций под процессы и услуги",
      "Работа в контуре компании",
  ...
  },
  {
    ...
    price: "300 000 ₽",
    desc: "Администраторы отрабатывают сценарии общения с пациентами. Кастомная настройка сценариев и логики тренировки под специфику клиники",
    includes: [
      "Сценарии под специфику клиники",
      ...
      "Кастомная настройка сценариев и ролей",
      "Работа в контуре компании"
    ],
  ...
  },
  {
    ...
    price: "250 000 ₽",
    desc: "Вся управленческая аналитика на одном листе, включая результаты роста компетенций сотрудников и их влияние на бизнес-показатели. Кастомная настройка показателей под клинику",
    includes: [
      ...
      "Кастомная настройка метрик и структуры дашборда",
      "Работа в контуре компании"
    ],
  ...
  },
  {
    ...
    price: "200 000 ₽",
    desc: "Получайте объективную оценку работы администраторов по индивидуальным критериям без ручного прослушивания звонков. Кастомная настройка отчётов и критериев под клинику",
    includes: [
      ...
      "Кастомная настройка критериев и логики оценки",
      "Работа в контуре компании"
    ],
...
const GREEN_ICON = "#5f9100";
const ICON_WALLET = svgIcon(
...
const ICON_TUNE = svgIcon(
  "M3 17v2h6v-2H3zM3 5v2h10V5H3zm10 16v-2h8v-2h-8v-2h-2v6h2zM7 9v2H3v2h4v2h2V9H7zm14 4v-2H11v2h10zm-6-4h2V7h4V5h-4V3h-2v6z",
  GREEN_ICON
);
const ICON_INFINITY = svgIcon(
...
const HERO_CHIPS: Array<{ icon: string; lines: [string, string] }> = [
  { icon: ICON_WALLET, lines: ["Окупаемость", "от 1 месяца"] },
  { icon: ICON_TUNE, lines: ["Индивидуальная", "настройка"] },
  { icon: ICON_INFINITY, lines: ["Бессрочное", "использование"] }
];
...
function setSeoMetadata() {
  ...
  document.title = "ИИ-тренажёр для клиник — Цифровая методология";

  const description =
    "Решение для роста выручки премиальных клиник: аналитика звонков, ИИ-тренажёр, оценка компетенций и дашборды.";
  const tags = [
    ["description", description],
    ["og:title", "ИИ-тренажёр для клиник — Цифровая методология"],
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView ref={scrollRef} stickyHeaderIndices={[0]} showsVerticalScrollIndicator={false}>
            ...
            <View style={[styles.heroGrid, isTablet && styles.oneColumn]}>
              <Reveal style={isTablet ? styles.heroCopyStacked : styles.heroCopy}>
                <View>
                  ...
                  <View style={[styles.heroChips, isMobile && styles.heroChipsMobile]}>
                    {HERO_CHIPS.map((chip) => (
                      <View key={chip.lines[0]} style={styles.heroChip}>
                        <View style={styles.heroChipIcon}>
                          <Image source={{ uri: chip.icon }} resizeMode="contain" style={styles.heroChipIconImg} />
                        </View>
        ...
        <Section id="trainer" register={register} containerStyle={containerStyle} sectionPadding={sectionPadding}>
          ...
          >
            Как работает ИИ-тренажёр
          </Text>
          <Text style={[styles.sub, styles.subWide]}>
            Тренажёр учит не «правильным фразам из скрипта», а живому разговору администратора с пациентом
          </Text>
        ...
        >
          ...
          <View style={styles.pricingBanner}>
            <Image source={{ uri: ICON_SHIELD }} style={styles.pricingBannerIcon} />
            <Text style={styles.pricingBannerText}>
              Все решения работают в контуре вашей компании. Кастомная настройка под процессы, услуги и цели клиники. Без
        ...
        >
          ...
          <View style={[styles.blogRow, isTablet && styles.oneColumn]}>
            <Pressable
              onPress={() => setArticleOpen(true)}
              style={(state) => [styles.blogCard, styles.blogArticleCard, isTablet && styles.stackChild, (state as { hovered?: boolean }).hovered && styles.tgCtaHover]}
            >
              <Text style={styles.blogMeta}>{BLOG[0][0]}</Text>
              <Text style={styles.blogTitle}>{BLOG[0][1]}</Text>
              <Text style={styles.blogText}>{BLOG[0][2]}</Text>
              <Text style={styles.blogLink}>{BLOG[0][3]}</Text>
            </Pressable>
            <Pressable
              onPress={() => openExternal("https://t.me/AItheraDM")}
              style={(state) => [styles.tgCard, isTablet && styles.stackChild, (state as { hovered?: boolean }).hovered && styles.tgCtaHover]}
            >
              <View style={styles.tgCardDecor} />
              <View style={styles.tgCardIcon}>
                <Image source={{ uri: ICON_TELEGRAM_W }} style={styles.tgCardIconImg} />
              </View>
              <Text style={styles.tgCardTitle}>Больше разборов, кейсов и обновлений — в Telegram</Text>
              <Text style={styles.tgCardSub}>Подпишитесь на канал Aithera, чтобы не пропускать новые материалы</Text>
              <View style={styles.tgCardBtn}>
                <Text style={styles.tgCardBtnText}>Перейти в канал</Text>
              </View>
            </Pressable>
          </View>
        </Section>
...
const styles = StyleSheet.create({
  ...
  decorSquareTop: { width: 130, height: 130, right: -78, top: -70 },
  decorSquareBottom: { width: 90, height: 90, left: -55, bottom: -55 },
  ...
  heroFlow: { gap: 10 },
  flowStep: { backgroundColor: "rgba(255,255,255,.1)", borderWidth: 1, borderColor: "rgba(255,255,255,.12)", borderRadius: 16, paddingVertical: 13, paddingHorizontal: 14 },
  flowStepHead: { flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 5 },
  flowNum: { width: 24, height: 24, borderRadius: 999, backgroundColor: LIME, color: NAVY, textAlign: "center", lineHeight: 24, fontSize: 13, fontWeight: "900", overflow: "hidden" },
  flowTitle: { flex: 1, color: "#fff", fontSize: 16, lineHeight: 19, fontWeight: "900" },
  flowText: { color: "rgba(255,255,255,.8)", fontSize: 13, lineHeight: 18 },
  ...
  heroChips: { flexDirection: "row", flexWrap: "wrap", gap: 14, marginTop: 24, alignItems: "stretch" },
  heroChipsMobile: { gap: 10 },
  heroChip: { flexGrow: 1, flexBasis: 150, minWidth: 0, backgroundColor: "#fff", borderWidth: 1, borderColor: LINE, borderRadius: 18, paddingVertical: 18, paddingHorizontal: 14, alignItems: "center", justifyContent: "center", gap: 12, ...shadow },
  heroChipIcon: { width: 44, height: 44, borderRadius: 13, backgroundColor: "#edf8ce", alignItems: "center", justifyContent: "center" },
  heroChipIconImg: { width: 24, height: 24 },
  heroChipText: { color: NAVY, fontSize: 14, lineHeight: 18, fontWeight: "800", textAlign: "center" },
  ...
  planNote: { flexDirection: "row", gap: 10, alignItems: "flex-start", backgroundColor: "rgba(156,240,0,.1)", borderWidth: 1, borderColor: "rgba(156,240,0,.5)", borderRadius: 14, padding: 14, marginBottom: 18 },
  planNoteIcon: { width: 18, height: 18, marginTop: 1, flexShrink: 0 },
  planNoteText: { flex: 1, minWidth: 0, color: LIME_2, fontSize: 13, lineHeight: 19, fontWeight: "700" },
  pricingHead: { flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", gap: 24, marginBottom: 28 },
  pricingTitle: { flex: 1, minWidth: 0, marginBottom: 0 },
  pricingShield: { flexDirection: "row", alignItems: "center", gap: 12, maxWidth: 420, backgroundColor: "#fff", borderWidth: 1, borderColor: LINE, borderRadius: 16, paddingVertical: 12, paddingHorizontal: 16, marginTop: 6, ...shadow },
  pricingShieldIcon: { width: 26, height: 26, flexShrink: 0 },
  pricingShieldText: { flex: 1, minWidth: 0, color: NAVY, fontSize: 13, lineHeight: 18, fontWeight: "800", textTransform: "uppercase", letterSpacing: 0.3 },
  pricingBanner: { flexDirection: "row", alignItems: "center", gap: 14, marginBottom: 28, backgroundColor: SOFT_2, borderRadius: 18, paddingVertical: 18, paddingHorizontal: 22 },
  pricingBannerIcon: { width: 26, height: 26, flexShrink: 0 },
  pricingBannerText: { flex: 1, minWidth: 0, color: NAVY, fontSize: 14, lineHeight: 20, fontWeight: "800", textTransform: "uppercase", letterSpacing: 0.3 },
  ...
  blogRow: { flexDirection: "row", gap: 20, alignItems: "stretch" },
  blogArticleCard: { flex: 1, minWidth: 0, justifyContent: "flex-start", ...(Platform.OS === "web" ? ({ cursor: "pointer", transitionDuration: "180ms", transitionProperty: "transform" } as object) : null) },
  tgCard: {
    flex: 1,
    minWidth: 0,
    backgroundColor: NAVY,
  ...
  tgCardDecor: { position: "absolute", width: 150, height: 150, backgroundColor: "#229ED9", right: -64, top: -54, opacity: 0.5, transform: [{ rotate: "45deg" }] },
  tgCardIcon: { width: 56, height: 56, borderRadius: 16, backgroundColor: "#229ED9", alignItems: "center", justifyContent: "center", marginBottom: 18, zIndex: 1 },
  tgCardIconImg: { width: 32, height: 32 },
  tgCardTitle: { color: "#fff", fontSize: 22, lineHeight: 28, fontWeight: "900", marginBottom: 10, zIndex: 1 },
  tgCardSub: { color: "rgba(255,255,255,.8)", fontSize: 15, lineHeight: 22, marginBottom: 22, zIndex: 1 },
  tgCardBtn: { alignSelf: "flex-start", backgroundColor: LIME, borderRadius: 999, paddingVertical: 13, paddingHorizontal: 24, zIndex: 1 },
  tgCardBtnText: { color: NAVY, fontSize: 15, fontWeight: "900" },
  ...
  blogTitle: { color: NAVY, fontSize: 22, lineHeight: 23, fontWeight: "900", textTransform: "uppercase", marginBottom: 10 },
  blogText: { color: MUTED, fontSize: 15, lineHeight: 23, marginBottom: 16 },
